...
class AES256Crypto:
    ...
    def __init__(self, key, mode=Mode.ECB, iv=None):
        """
    ...
    def pad(self, text, _len=16):
        text = text.encode()
        remainder = len(text) % _len
        if remainder != 0:
            text += chr(_len - remainder).encode() * (_len - remainder) if remainder != 0 else ''.encode()
        return text
...
aes = AES256Crypto(key, Mode.CBC, iv)
